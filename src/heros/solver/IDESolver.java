...
public class IDESolver<N,D,M,V,I extends CustomInterProceduralCFG<N, M>> {
	...
	public boolean PRINT_LOGS = false;
	...
	private void startProfiling() {
		...
		for(Cell<N, N, Map<D, Set<D>>> cell: intraEdges) {
			...
			for(Entry<D, Set<D>> edge: cell.getValue().entrySet()) {
				D key = edge.getKey();
				Set<D> value = edge.getValue();
				numberOfIntraPathEdges += value.size();
				if(value.contains(key)){
					numgenFacts += value.size() - 1;
				}
				else{
		...
		for(Cell<N, N, Map<D, Set<D>>> cell: interEdges) {
			...
			if(icfg.isCallStmt(rowKey)){
				for (Entry<D, Set<D>> edge : cell.getValue().entrySet()) {
					numInterPathEdges += edge.getValue().size();
					for(D src: edge.getValue()) {
						if(!src.equals(zeroValue)){
							numgenFacts++;
						}
						//TODO@Palani Process summary just as an interproc edge
					}
				}
			}
